...
// Next.js App Router 스타일 (ESM, Web Request/Response API)
...
function buildNaverHeaders() {
  ...
  return { 'Content-Type': 'application/json; charset=UTF-8', 'X-Timestamp': timestamp, 'X-API-KEY': apiKey, 'X-Customer': String(customerId), 'X-Signature': signature }
}
...
const baseHandler = createMcpHandler(
  (server) => {
    ...
    server.registerTool('save_keyword_data', {
      ...
      description: 'naver_keyword_volume 조회 결과를 blog_keyword_stats에 저장한다. hint는 카테고리명으로 지정한다.',
      inputSchema: {
    ...
    server.registerTool('add_publish_log', {
      ...
      inputSchema: {
        ...
        angle: z.string().describe('키워드 각도'),
    ...
    server.registerTool('create_blog_post', {
      title: '블로그 글 실제 발행',
      description: '작성한 글 본문을 blog_posts 테이블에 발행한다. 발행 후 반드시 add_publish_log와 mark_keyword_used도 호출한다.',
      inputSchema: {
        ...
        content: z.string().describe('본문 마크다운 전체'),
