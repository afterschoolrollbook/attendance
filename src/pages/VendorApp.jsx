...
export function VendorApp({ vendorSession: initSession, onLogout }) {
    ...
    try {
      ...
      for (const [sName, sData] of Object.entries(subjectMap)) {
        ...
        for (const [pName, plans] of Object.entries(sData.products)) {
          ...
          if (!product) { const n={id:uid(),vendorId,subjectId:subject.id,name:pName,createdAt:now()}; await DB.saveProduct(n); product=n; myP.push(n); pc++ }
  ...
  const saveProduct = async () => {
    ...
    await DB.saveProduct({id:productId,vendorId,subjectId:productForm.subjectId,name:productForm.name,createdAt:isEdit?(editingProduct?.createdAt||now()):now()})
