...
function VendorProductsPage({ vendorId, vendorSession, subjects, products, onReload }) {
  ...
  const downloadSample = async () => {
    const rows = [
      ...
      ['한국교구업체', '김철수', '010-1234-5678', '로봇', 'A', '큐보', 1, 1, '큐보 1단계 1차시', '배터리'],
      ['한국교구업체', '김철수', '010-1234-5678', '로봇', 'A', '큐보', 1, 2, '큐보 1단계 2차시', '배터리'],
      ['한국교구업체', '김철수', '010-1234-5678', '로봇', 'A', '큐보', 2, 1, '큐보 2단계 1차시', ''],
      ['한국교구업체', '김철수', '010-1234-5678', '보드게임', 'B', '할리갈리', 1, 1, '할리갈리 1차시', ''],
      ['한국교구업체', '김철수', '010-1234-5678', '미술', 'C', '수채화', 1, 1, '수채화 기초', '물감,붓'],
    ]
    const wb = XLSX.utils.book_new()
    const ws = XLSX.utils.aoa_to_sheet(rows)
