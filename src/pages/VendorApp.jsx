/**
 * VendorApp.jsx
 * 업체 전용 앱 — Supplies.jsx 교구업체 탭 구조 완전 동일
 * 연간지도안 / 차시별지도안 / 홍보물 파일 업로드 포함
 */
    ...
    <div style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 14px', background:C.card, borderRadius:'9px', border:`1.5px solid ${noFile?'#fca5a5':C.border}` }}>
      ...
      <div style={{ flex:1, minWidth:0 }}>
        ...
        <div style={{ fontSize:'11px', marginTop:'2px', display:'flex', gap:'8px', flexWrap:'wrap', alignItems:'center' }}>
          ...
          {noFile
            ? <span style={{ color:C.danger, fontWeight:600 }}>파일 업로드가 필요합니다</span>
      ...
      <div style={{ display:'flex', gap:'5px', flexShrink:0 }}>
        ...
        {onEdit && (
          <button type="button" onClick={()=>onEdit(item)}
            style={{ padding:'4px 10px', borderRadius:'6px', border:`1px solid ${C.primary}`, background:'#fff7ed', color:C.primary, fontSize:'11px', fontWeight:600, cursor:'pointer', fontFamily:'Noto Sans KR, sans-serif' }}>수정</button>
        )}
...
function ProductsPage({ vendorId, vendorSession, subjects, products, contents, files, onReload }) {
  ...
  const [fileMode,      setFileMode]      = useState('annual') // annual | session | promo
  ...
  const handleBulkUpload = async (e) => {
    ...
    try {
      ...
      let sc=0, pc=0, cc=0
      const curS=(await dbCall('getAll','hqVendorSubjects'))||[]; const curP=(await dbCall('getAll','hqVendorProducts'))||[]; const curC=(await dbCall('getAll','hqVendorContents'))||[]
      const myS=curS.filter(s=>s.vendorId===vendorId&&!s._deleted); const myP=curP.filter(p=>p.vendorId===vendorId&&!p._deleted); const myC=curC.filter(c=>!c._deleted)
      ...
      onReload(); success(`과목 ${sc}개, 교구 ${pc}개, 차시 ${cc}개 등록 완료!`)
  ...
  const openProductModal = (subjectId, existing=null) => {
    ...
    if (existing) {
      ...
      setProductForm({id:null,name:'',maxStage:10,sessionsPerStage:12,alertSession:3}); setStageSessionTitles(titles); setEditingProduct(null)
    }
  ...
  const saveProduct = async () => {
    ...
    await DB.saveProduct({id:productId,vendorId,subjectId:productSubjectId,name:productForm.name,maxStage:productForm.maxStage,sessionsPerStage:productForm.sessionsPerStage,alertSession:productForm.alertSession,createdAt:isEdit?(editingProduct?.createdAt||now()):now()})
    ...
    onReload(); setProductModal(false); success(isEdit?'수정되었습니다.':'교구가 등록되었습니다.')
  }

  const deleteProduct = (p) => {
    setDeleteConfirm({ msg:`"${p.name}" 교구를 삭제하시겠습니까?`, onOk:async()=>{ await DB.delProduct(p.id); onReload(); success('삭제가 완료되었습니다.') }})
  }
  ...
  const saveFileFn = async () => {
    ...
    const autoTitle = product
      ? (fileMode==='promo' ? `${product.name} 홍보물` : fileMode==='session' ? `${product.name} ${fileStage}단계 차시별 지도안` : `${product.name} 연간지도안`)
      : (fileMode==='promo' ? '홍보물' : fileMode==='session' ? '차시별지도안' : '연간지도안')
    setUploading(true)
    try {
      let fileUrl=null, fileName=null
      if (modalFile) {
        fileUrl = await uploadToStorage(vendorId, fileMode, modalFile)
        fileName = modalFile.name
      }
      if (fileEditItem) {
        await DB.saveFile({...fileEditItem, fileType:fileMode, title:autoTitle, stage:fileStage||null, productId:fileProductId, fileUrl:fileUrl||fileEditItem.fileUrl, fileName:fileName||fileEditItem.fileName})
      } else {
        await DB.saveFile({id:uid(), vendorId, productId:fileProductId, fileType:fileMode, title:autoTitle, stage:fileStage||null, fileUrl, fileName, createdAt:now()})
      }
      onReload(); setFileModal(false); setModalFile(null); setFileEditItem(null); success(fileEditItem?'수정이 완료되었습니다.':'등록이 완료되었습니다.')
    } catch(e) { toastError('업로드 실패: '+e.message) }
    finally { setUploading(false) }
  }

  const deleteFile = (id) => {
    setDeleteConfirm({ msg:'이 파일을 삭제하시겠습니까?', onOk:async()=>{ await DB.delFile(id); onReload(); success('삭제가 완료되었습니다.') }})
  }
    ...
    <div style={{ padding:'24px', maxWidth:'1100px', fontFamily:'Noto Sans KR, sans-serif' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px', flexWrap:'wrap', gap:'12px' }}>
        ...
        <div style={{ display:'flex', gap:'8px' }}>
          <button type="button" onClick={downloadSampleExcel} style={{ padding:'6px 12px', borderRadius:'7px', border:`1.5px solid ${C.blue}`, background:'#eff6ff', color:C.blue, fontSize:'12px', fontWeight:600, cursor:'pointer', fontFamily:'Noto Sans KR, sans-serif' }}>📋 샘플 다운로드</button>
          ...
          <button type="button" onClick={()=>openProductModal(subjects[0]?.id)} style={{ padding:'6px 14px', borderRadius:'7px', border:`1.5px solid ${C.primary}`, background:'#fff7ed', color:C.primary, fontSize:'12px', fontWeight:700, cursor:'pointer', fontFamily:'Noto Sans KR, sans-serif' }}>+ 교구 등록</button>
          <button type="button" onClick={()=>{ setNewSubject(''); setSubjectModal(true) }} style={{ padding:'6px 14px', borderRadius:'7px', border:'none', background:C.primary, color:'#fff', fontSize:'12px', fontWeight:700, cursor:'pointer', fontFamily:'Noto Sans KR, sans-serif' }}>+ 과목 추가</button>
        </div>
      </div>
      ...
      {subjects.length===0 && (
        <div style={{ textAlign:'center', padding:'60px', color:C.muted }}>
          ...
          <div style={{ fontSize:'13px', marginTop:'6px' }}><strong>일괄 등록</strong> 또는 <strong>+ 과목 추가</strong>로 시작하세요.</div>
        </div>
      )}
                  ...
                  <div style={{ padding:'14px 18px' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
                      ...
                      <div style={{ display:'flex', gap:'6px' }}>
                        {sProducts.length>0 && <button type="button" onClick={()=>downloadProductsExcel(vendor.name||'', sProducts, contents)} style={{ padding:'4px 10px', borderRadius:'6px', border:`1.5px solid ${C.success}`, background:'#f0fdf4', color:C.success, fontSize:'12px', fontWeight:600, cursor:'pointer', fontFamily:'Noto Sans KR, sans-serif' }}>⬇ 다운로드</button>}
                        ...
                        <button type="button" onClick={()=>openProductModal(s.id)} style={{ padding:'4px 10px', borderRadius:'6px', border:`1.5px solid ${C.primary}`, background:'#fff7ed', color:C.primary, fontSize:'12px', fontWeight:600, cursor:'pointer', fontFamily:'Noto Sans KR, sans-serif' }}>+ 교구 등록</button>
                      </div>
                    </div>
                            ...
                            return (
                              <div key={p.id} style={{ background:C.bg, borderRadius:'10px', border:`1px solid ${C.border}`, overflow:'hidden' }}>
                                ...
                                <div style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 14px' }}>
                                  ...
                                  <button type="button" onClick={()=>openProductModal(s.id,p)} style={{ padding:'4px 8px', borderRadius:'6px', border:`1px solid ${C.primary}`, background:'#fff7ed', color:C.primary, fontSize:'11px', cursor:'pointer', fontFamily:'Noto Sans KR, sans-serif' }}>수정</button>
          ...
          <div style={{ background:'#fff', borderRadius:'16px', width:'500px', maxWidth:'95vw', maxHeight:'85vh', display:'flex', flexDirection:'column', boxShadow:'0 8px 40px rgba(0,0,0,0.18)' }}>
            <div style={{ padding:'20px 24px', borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div style={{ fontSize:'16px', fontWeight:700, color:C.text }}>
                {fileMode==='annual'?'📅 연간지도안':fileMode==='session'?'📝 차시별지도안':'🖼 홍보물'} {fileEditItem?'수정':'등록'}
              </div>
              ...
              {fileMode==='session' && (
                <div>
                  ...
                  <select value={fileStage} onChange={e=>setFileStage(e.target.value)} style={{ ...iSt, background:'#fff' }}>
                    ...
                    {Array.from({length:(products.find(p=>p.id===fileProductId)?.maxStage||10)},(_,i)=>i+1).map(s=><option key={s} value={s}>{s}단계</option>)}
                  </select>
                </div>
              )}
              {/* 파일 첨부 */}
