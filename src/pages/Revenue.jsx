                        ...
                        <div key={p.id} style={{ padding:'8px 10px', borderRadius:'8px', background:'#f0fdf4', border:'1px solid #86efac' }}>
                          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                            ...
                            <div style={{ display:'flex', alignItems:'center', gap:'6px', flexShrink:0 }}>
                              ...
                              <Btn size='sm' variant='outlineDanger' onClick={() => confirm('입금 내역을 삭제할까요?', () => deletePayment(p.id), { icon: '🗑', confirmLabel: '삭제' })}>삭제</Btn>
                            </div>
                          </div>
                        </div>
                      )
                                  ...
                                  {tPays.length===0
                                    ...
                                    : [...tPays].sort((a,b)=>(a.date||'').localeCompare(b.date||'')).map(p=>(
                                      <div key={p.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 0', borderBottom:`1px solid #f3f4f6` }}>
                                        ...
                                        <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                                          <span style={{ fontSize:'13px', fontWeight:700, color:C.success }}>+{fmt(p.amount)}원</span>
                                          <Btn size='sm' variant='outlineDanger' onClick={() => confirm('삭제할까요?', () => deletePayment(p.id), { icon: '🗑', confirmLabel: '삭제' })}>삭제</Btn>
                                        </div>
                                      </div>
                                    ))
                                  }
        ...
        <div onClick={e=>{ if(e.target===e.currentTarget) setFeeModal(false) }}
          ...
          <div style={{ background:'#fff', borderRadius:'16px', width:'100%', maxWidth:'440px', boxShadow:'0 20px 60px rgba(0,0,0,0.2)', overflow:'hidden' }}>
            ...
            <div style={{ padding:'20px', display:'flex', flexDirection:'column', gap:'14px' }}>
              ...
              <div style={{ display:'flex', gap:'8px' }}>
                <Btn onClick={saveFeeForm} full>저장</Btn>
