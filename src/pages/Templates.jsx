...
function PreviewModal({ doc, color, onClose, onAttach, onUpdate }) {
  ...
  const handleSave=async()=>{
    ...
    if(file){
      ...
      patch.fileName=file.name;patch.fileType=getFileType(file.name)
    }
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:1100,background:'rgba(0,0,0,0.45)',display:'flex',alignItems:'center',justifyContent:'center'}} onClick={e=>{if(e.target===e.currentTarget)onClose()}}>
      <div style={{background:'#fff',borderRadius:'16px',padding:'28px',width:'420px',maxWidth:'92vw',boxShadow:'0 20px 60px rgba(0,0,0,0.18)',fontFamily:'Noto Sans KR, sans-serif'}}>
        ...
        {/* 헤더 */}
        <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'22px'}}>
          <span style={{fontSize:'14px',fontWeight:600,color:'#111827',flex:1}}>{editMode?'서류 수정':doc.title}</span>
          {!editMode&&<button onClick={()=>setEditMode(true)} style={{padding:'5px 12px',borderRadius:'7px',border:'1.5px solid '+color+'50',background:color+'10',color:color,fontSize:'12px',fontWeight:600,cursor:'pointer',fontFamily:'Noto Sans KR, sans-serif'}}>✏️ 수정</button>}
          <button onClick={onClose} style={{background:'none',border:'none',cursor:'pointer',color:'#9ca3af',fontSize:'20px',lineHeight:1,padding:0}}>×</button>
        </div>

        {/* 미리보기 모드 */}
        {!editMode&&(
          <>
            {doc.fileName&&<div style={{fontSize:'12px',color:'#6b7280',background:'#f9fafb',padding:'8px 12px',borderRadius:'8px',marginBottom:'14px'}}>📎 {doc.fileName}</div>}
            <div style={{borderRadius:'10px',overflow:'hidden',border:'1px solid #e5e7eb',minHeight:'200px',display:'flex',alignItems:'center',justifyContent:'center',background:'#f9fafb',marginBottom:'16px'}}>
              {!doc.fileData
                ?(<div style={{textAlign:'center',padding:'40px'}}><div style={{fontSize:'40px',marginBottom:'10px'}}>📭</div><div style={{fontSize:'13px',fontWeight:600,color:'#b45309'}}>파일이 첨부되지 않았습니다</div><div style={{fontSize:'12px',marginTop:'4px',color:'#9ca3af'}}>수정 버튼을 눌러 파일을 첨부하세요</div></div>)
        ...
        {editMode&&(
          <>
            ...
            <div style={{display:'flex',gap:'8px',justifyContent:'flex-end'}}>
              <button onClick={()=>setEditMode(false)} style={{padding:'9px 18px',borderRadius:'8px',border:'1.5px solid #e5e7eb',background:'#fff',fontSize:'13px',fontWeight:600,color:'#6b7280',cursor:'pointer',fontFamily:'Noto Sans KR, sans-serif'}}>취소</button>
