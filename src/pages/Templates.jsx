...
function getDayFromTitle(title) {
  return DAY_ORDER[title?.split(' ')[0]] ?? 99
}
...
const ds = () => ({
  ...
  fontSize: '13px', color: '#111827', outline: 'none', cursor: 'pointer',
...
const lbl = (text, opt) => (
  <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#374151', marginBottom: '6px' }}>
    {text}{opt && <span style={{ color: '#9ca3af', fontWeight: 400 }}> (선택)</span>}
  </label>
)
...
function AddModal({ cat, onClose, onSave }) {
  ...
  const [p1, setP1]           = useState('해당없음')
  const [p2, setP2]           = useState('해당없음')
  ...
  const handleSave = async () => {
    ...
    if (file) {
      fileData = await new Promise(r => { const rd = new FileReader(); rd.onload = e => r(e.target.result); rd.readAsDataURL(file) })
      fileName = file.name; fileType = getFileType(file.name)
    }
...
function PreviewModal({ doc, color, onClose, onAttach }) {
  ...
  const handleDownload = () => {
    const a = document.createElement('a'); a.href = doc.fileData; a.download = doc.fileName; a.click()
  }
  const handleAttach = async () => {
    if (!newFile) return
    setAttaching(true)
    const fileData = await new Promise(r => { const rd = new FileReader(); rd.onload = e => r(e.target.result); rd.readAsDataURL(newFile) })
    onAttach({ fileData, fileName: newFile.name, fileType: getFileType(newFile.name) })
    success('파일이 첨부되었습니다.')
    setAttaching(false); onClose()
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1100, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', width: '520px', maxWidth: '92vw', boxShadow: '0 24px 64px rgba(0,0,0,0.22)', fontFamily: 'Noto Sans KR, sans-serif', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        {doc.fileName && !noFile && <div style={{ fontSize: '12px', color: '#6b7280', background: '#f9fafb', padding: '8px 12px', borderRadius: '8px' }}>📎 {doc.fileName}</div>}

        <div style={{ borderRadius: '10px', overflow: 'hidden', border: '1px solid #e5e7eb', minHeight: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f9fafb' }}>
          {noFile ? (
            <div style={{ textAlign: 'center', padding: '40px' }}>
              <div style={{ fontSize: '40px', marginBottom: '10px' }}>📭</div>
              <div style={{ fontSize: '13px', fontWeight: 600, color: '#b45309' }}>서류 파일이 첨부되지 않았습니다</div>
              <div style={{ fontSize: '12px', marginTop: '4px', color: '#9ca3af' }}>아래에서 파일을 첨부해주세요</div>
            </div>
          ) : doc.fileType === 'image' ? (
            <img src={doc.fileData} alt={doc.title} style={{ maxWidth: '100%', maxHeight: '380px', objectFit: 'contain', display: 'block' }} />
          ) : doc.fileType === 'pdf' ? (
            <iframe src={doc.fileData} title={doc.title} style={{ width: '100%', height: '380px', border: 'none' }} />
          ) : (
            <div style={{ textAlign: 'center', padding: '40px' }}>
              <div style={{ fontSize: '48px', marginBottom: '12px' }}>{doc.fileType === 'hwp' ? '📄' : doc.fileType === 'excel' ? '📊' : '📁'}</div>
        ...
        {noFile && (
          <div>
            ...
            <button onClick={() => fileRef.current?.click()} style={{ width: '100%', padding: '9px 12px', border: '1.5px dashed ' + (newFile ? color : '#d1d5db'), borderRadius: '8px', background: newFile ? color + '08' : '#fafafa', cursor: 'pointer', fontSize: '13px', color: newFile ? color : '#9ca3af', fontFamily: 'Noto Sans KR, sans-serif', textAlign: 'left', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
          ...
          {noFile ? (
            <button onClick={handleAttach} disabled={!newFile || attaching} style={{ padding: '9px 22px', borderRadius: '8px', border: 'none', background: newFile ? color : '#d1d5db', fontSize: '13px', fontWeight: 700, color: '#fff', cursor: newFile ? 'pointer' : 'not-allowed', fontFamily: 'Noto Sans KR, sans-serif' }}>
              {attaching ? '첨부 중...' : '📎 파일 첨부'}
            </button>
          ) : (
            <button onClick={handleDownload} style={{ padding: '9px 22px', borderRadius: '8px', border: 'none', background: color, fontSize: '13px', fontWeight: 700, color: '#fff', cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif' }}>⬇️ 다운로드</button>
          )}
        </div>
      </div>
    </div>
...
function DocChip({ doc, color, onDelete, onUpdate }) {
  ...
  return (
    <>
      ...
      {show && <PreviewModal doc={doc} color={color} onClose={() => setShow(false)} onAttach={p => onUpdate(doc.id, p)} />}
    </>
  )
}
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1200, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div style={{ background: '#fff', borderRadius: '16px', padding: '28px', width: '420px', boxShadow: '0 20px 60px rgba(0,0,0,0.18)', fontFamily: 'Noto Sans KR, sans-serif', display: 'flex', flexDirection: 'column', gap: '18px' }}>
        ...
        <div>
          ...
          <input autoFocus value={label} onChange={e => setLabel(e.target.value)} onKeyDown={e => e.key === 'Enter' && label.trim() && (onAdd({ key: 'custom_' + Date.now(), label: label.trim(), icon, color, type }), onClose())} placeholder="예) 행정서류"
        ...
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ padding: '9px 18px', borderRadius: '8px', border: '1.5px solid #e5e7eb', background: '#fff', fontSize: '13px', fontWeight: 600, color: '#6b7280', cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif' }}>취소</button>
          <button onClick={() => { if (label.trim()) { onAdd({ key: 'custom_' + Date.now(), label: label.trim(), icon, color, type }); onClose() } }} disabled={!label.trim()}
            style={{ padding: '9px 22px', borderRadius: '8px', border: 'none', background: label.trim() ? color : '#d1d5db', fontSize: '13px', fontWeight: 700, color: '#fff', cursor: label.trim() ? 'pointer' : 'not-allowed', fontFamily: 'Noto Sans KR, sans-serif' }}>추가</button>
        </div>
      </div>
    </div>
  )
...
export function Templates({ user }) {
  ...
  const [customCats, setCustomCats] = useState(() => CustomCategoriesDB.byTeacher(user.id))
  ...
  const reloadCats = () => setCustomCats(CustomCategoriesDB.byTeacher(user.id))
  ...
  const handleAddCat    = (cat) => {
    CustomCategoriesDB.insert({ id: uid(), ...cat, teacherId: user.id, createdAt: now() })
    reloadCats()
  ...
  const docsFor = (catKey) =>
    docs.filter(d => d.category === catKey).sort((a, b) => getDayFromTitle(a.title) - getDayFromTitle(b.title))

  const allCats    = [...CATEGORIES, ...customCats]
  const grouped    = TYPE_ORDER.reduce((acc, tk) => { const c = allCats.filter(x => x.type === tk); if (c.length) acc.push({ tk, c }); return acc }, [])
  const untyped    = allCats.filter(c => !c.type || !DOC_TYPES[c.type])
  if (untyped.length) grouped.push({ tk: null, c: untyped })

  const renderRow = (cat, isLast) => {
    const catDocs = docsFor(cat.key)
    const isCustom = !!cat.id
    return (
      <div key={cat.key} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '11px 20px', borderBottom: isLast ? 'none' : '1px solid #f3f4f6', minHeight: '52px' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '6px 12px', borderRadius: '8px', background: cat.color + '12', border: '1px solid ' + cat.color + '30', fontSize: '12px', fontWeight: 700, color: cat.color, whiteSpace: 'nowrap', flexShrink: 0, width: '148px', justifyContent: 'center' }}>
          <span>{cat.icon}</span><span>{cat.label}</span>
        </div>
        <button onClick={() => setModalCat(cat)} style={{ width: '28px', height: '28px', borderRadius: '7px', background: cat.color + '18', border: '1.5px solid ' + cat.color + '40', color: cat.color, fontSize: '18px', fontWeight: 700, cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          onMouseEnter={e => { e.currentTarget.style.background = cat.color + '30' }}
          onMouseLeave={e => { e.currentTarget.style.background = cat.color + '18' }}>+</button>
        ...
        {isCustom && (
          <button onClick={() => handleDeleteCat(cat)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#d1d5db', fontSize: '15px', flexShrink: 0, padding: '2px' }}
            onMouseEnter={e => e.currentTarget.style.color = '#ef4444'}
        ...
        {grouped.map(({ tk, c }) => {
          ...
          return (
            <div key={tk || 'etc'} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden' }}>
              ...
              {c.map((cat, ci) => renderRow(cat, ci === c.length - 1))}
            </div>
          )
        })}
