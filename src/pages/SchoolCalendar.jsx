 ...
 * SchoolCalendar.jsx
 * 학교 담당자용 연간 수업 달력
 * - 연도/운영방식/요일/방학 입력 → 자동 일정 생성
 * - 공휴일 자동 세팅
 * - 달력에서 휴일/보강 직접 수정
 */
import React, { useState, useEffect } from 'react'
import { getDayLabel } from '../lib/utils.js'
...
// ── 월별 달력 컴포넌트
function MonthCalendar({ year, month, sessionMap, cancelledSet, cancelledDates, makeupDates, termMap, onDateClick, vacationSet }) {
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:'2px' }}>
        {cells.map((day, idx) => {
          ...
          // 보강일
          if (isMakeup) {
            const mi = makeupDates.find(m=>m.date===dateStr)
            return (
              <button key={day} onClick={() => onDateClick(dateStr,'makeup')}
                ...
                {sessInfo && <div style={{ fontSize:'9px', color:'#fff', background:'#3b82f6', borderRadius:'3px', padding:'0 2px', marginTop:'1px' }}>{sessInfo.total}차시</div>}
              </button>
            )
          }
          ...
          if (sessInfo) {
            return (
              <button key={day} onClick={() => onDateClick(dateStr,'session')}
                style={{ padding:'3px 2px', borderRadius:'7px', border:'none', cursor:'pointer',
                  background: tc?.bg||'#fff7ed', outline:`1.5px solid ${tc?.border||'#f97316'}`, outlineOffset:'-1px',
                  textAlign:'center', fontFamily:'Noto Sans KR, sans-serif' }}>
                <div style={{ fontSize:'12px', fontWeight:700, color:isSun?'#ef4444':isSat?'#3b82f6':'#111827' }}>{day}</div>
                <div style={{ fontSize:'10px', color:tc?.text||'#ea580c', fontWeight:700, lineHeight:1.3 }}>{sessInfo.total}차시</div>
...
export function SchoolCalendar({ cls, onUpdate, session }) {
  ...
  const [termType,   setTermType]   = useState(cls?.termType || 'semester') // 'semester'|'quarter'
  ...
  // 분기제
  const [quarters,   setQuarters]   = useState(cls?.quarters || 4) // 2~4
  const [qEnds,      setQEnds]      = useState(cls?.qEnds    || ['','','','']) // 각 분기 종료일
  ...
  useEffect(() => {
    ...
    setTermType(cls.termType || 'semester')
    setDays(cls.days || [])
    setTitle(cls.title || '')
  ...
  // 전체 수업일 계산 (3월~해당 연도 12월 말, 방학/공휴일/휴일 제외)
  ...
  // 텀 구분
  let termBoundaries = [] // [{start, end, label}]
  if (termType === 'semester') {
    // 1학기: 3월~sem1End, 2학기: 복학일~sem2End
  ...
  // 텀별 수업일 배분 (각 텀 안에서 4주 단위로 세부텀 구성)
  const sessionMap = {}
  const termMap    = {}
  let globalIdx = 1
  let termNum = 1

  termBoundaries.forEach(boundary => {
    const termSessions = allSessionDates.filter(d => d >= boundary.start && d <= boundary.end)
    // 4주(4회)씩 세부텀
    let subTermIdx = 1
    let subTermCount = 1
    termSessions.forEach(d => {
      sessionMap[d] = { total: globalIdx++, termNum, termSess: subTermCount }
      termMap[d] = termNum
      subTermCount++
      if (subTermCount > 4) { subTermCount = 1; subTermIdx++; termNum++ }
    })
    if (subTermCount > 1) termNum++ // 마지막 세부텀 마무리
  })
  ...
  const handleRestore = () => {
    ...
    setCancelledDates(updC); setMakeupDates(updM)
    setShowAction(false)
  ...
  return (
    <div style={{ fontFamily:'Noto Sans KR, sans-serif' }}>
      ...
      {/* ══ 설정 패널 ══ */}
      <div style={{ background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:'14px', padding:'20px', marginBottom:'16px' }}>
        <div style={{ fontSize:'14px', fontWeight:800, color:'#1e3a5f', marginBottom:'16px' }}>📋 기본 설정</div>
        ...
        <div style={{ marginBottom:'14px' }}>
          ...
          <div style={{ display:'flex', gap:'8px' }}>
            {[{v:'semester',l:'학기제 (1·2학기)'},{v:'quarter',l:'분기제'}].map(({v,l}) => (
              <button key={v} onClick={()=>setTermType(v)} style={{ padding:'8px 16px', borderRadius:'9px', border:`1.5px solid ${termType===v?'#1e3a5f':'#e5e7eb'}`, background:termType===v?'#1e3a5f':'#fff', color:termType===v?'#fff':'#374151', fontSize:'13px', fontWeight:termType===v?700:400, cursor:'pointer', fontFamily:'Noto Sans KR, sans-serif' }}>{l}</button>
            ))}
            {termType==='quarter' && [2,3,4].map(n => (
              <button key={n} onClick={()=>setQuarters(n)} style={{ padding:'8px 14px', borderRadius:'9px', border:`1.5px solid ${quarters===n?'#f97316':'#e5e7eb'}`, background:quarters===n?'#f97316':'#fff', color:quarters===n?'#fff':'#374151', fontSize:'13px', fontWeight:quarters===n?700:400, cursor:'pointer', fontFamily:'Noto Sans KR, sans-serif' }}>{n}분기</button>
            ))}
          </div>
        </div>
            ...
            {['월','화','수','목','금','토','일'].map(d => {
              ...
              return (
                <button key={d} onClick={() => setDays(sel ? days.filter(x=>x!==d) : [...days,d])}
                  style={{ width:'38px', height:'38px', borderRadius:'9px', border:'none', cursor:'pointer', fontSize:'14px', fontWeight:700, background:sel?'#1e3a5f':'#f3f4f6', color:sel?'#fff':'#374151', fontFamily:'Noto Sans KR, sans-serif' }}>{d}</button>
              )
            })}
          </div>
        </div>
        ...
        {termType === 'semester' && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', marginBottom:'14px' }}>
            <div style={{ background:'#eff6ff', borderRadius:'10px', padding:'12px' }}>
              ...
              <div style={{ fontSize:'11px', color:'#6b7280', marginBottom:'3px' }}>시작: {String(year)}-03-01 (자동)</div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:'10px', marginBottom:'14px' }}>
            {Array.from({length:quarters},(_,i) => (
              <div key={i} style={{ background: getTermColor(i+1).bg, border:`1.5px solid ${getTermColor(i+1).border}`, borderRadius:'10px', padding:'12px' }}>
                ...
                <label style={{ fontSize:'11px', color:'#6b7280', display:'block', marginBottom:'3px' }}>종료일</label>
                <input type="date" value={qEnds[i]||''} onChange={e=>{ const n=[...qEnds]; n[i]=e.target.value; setQEnds(n) }} onBlur={()=>saveAll()}
        ...
        <div style={{ display:'flex', gap:'10px', alignItems:'center', flexWrap:'wrap' }}>
          <button onClick={autoSetHolidays} style={{ padding:'8px 16px', borderRadius:'9px', border:'1.5px solid #f97316', background:'#fff7ed', color:'#ea580c', fontSize:'13px', fontWeight:700, cursor:'pointer', fontFamily:'Noto Sans KR, sans-serif' }}>
            🗓️ {year}년 공휴일 자동 추가
          </button>
          <button onClick={()=>saveAll()} style={{ padding:'8px 20px', borderRadius:'9px', border:'none', background:'#1e3a5f', color:'#fff', fontSize:'13px', fontWeight:700, cursor:'pointer', fontFamily:'Noto Sans KR, sans-serif' }}>
            💾 저장
          </button>
          {totalSessions > 0 && (
            <span style={{ fontSize:'13px', color:'#6b7280' }}>
              총 <strong style={{color:'#1e3a5f'}}>{totalSessions}회</strong> 수업 · <strong style={{color:'#f97316'}}>{totalTerms}텀</strong>
            </span>
          )}
        </div>
      </div>
      ...
      {totalSessions > 0 && (
        <div style={{ display:'flex', gap:'6px', flexWrap:'wrap', marginBottom:'12px' }}>
          ...
          <span style={{ fontSize:'12px', color:'#6b7280', display:'flex', alignItems:'center', marginLeft:'4px' }}>
            ...
            {vacationDates.size > 0 && ` · 방학 ${vacationDates.size}일`}
      ...
      {/* ══ 휴일 직접 추가 ══ */}
      <div style={{ background:'#fafafa', border:'1px solid #e5e7eb', borderRadius:'12px', padding:'14px 16px', marginBottom:'14px' }}>
        <div style={{ fontSize:'13px', fontWeight:700, color:'#374151', marginBottom:'10px' }}>📌 휴일 직접 추가 (개교기념일 등)</div>
