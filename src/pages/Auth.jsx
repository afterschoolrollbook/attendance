...
function useKakaoAuth(onSuccess, restApiKey) {
  const loginWithKakao = () => {
    if (!restApiKey) { authToast('카카오 앱 키가 설정되지 않았습니다. 관리자 → 서비스설정 → 소셜 로그인에서 등록하세요.'); return }
    ...
    const handleMessage = async (e) => {
      ...
      try {
        ...
        authToast('카카오 로그인에 실패했습니다: ' + err.message)
      }
    }
...
function useNaverAuth(onSuccess, clientId) {
  const loginWithNaver = () => {
    if (!clientId) { authToast('네이버 클라이언트 ID가 설정되지 않았습니다. 관리자 → 서비스설정 → 소셜 로그인에서 등록하세요.'); return }
...
export function Auth({ onLogin }) {
  const { toasts, error: authToast, success: authSuccess } = useToast()
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {googleConfigured ? (
                        ...
                        <SocialBtn icon="🔵" label="Google로 계속하기" color="#4285F4" bg="#fff" border="#dadce0"
                          onClick={() => authToast('Google 로그인을 사용하려면 관리자 → 서비스설정 → 소셜 로그인에서 등록하세요.')} />
                      )}
                      <SocialBtn icon="💛" label="카카오로 계속하기" color="#3C1E1E" bg="#FEE500" border="#FEE500"
                        onClick={kakaoConfigured ? loginWithKakao : () => authToast('카카오 로그인을 사용하려면 관리자 → 서비스설정 → 소셜 로그인에서 등록하세요.')} />
                      <SocialBtn icon="🟢" label="네이버로 계속하기" color="#fff" bg="#03C75A" border="#03C75A"
                        onClick={naverConfigured ? loginWithNaver : () => authToast('네이버 로그인을 사용하려면 관리자 → 서비스설정 → 소셜 로그인에서 등록하세요.')} />
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      ...
                      {googleConfigured ? (
                        ...
                        <SocialBtn icon="🔵" label="Google로 간편가입" color="#4285F4" bg="#fff" border="#dadce0"
                          onClick={() => authToast('관리자 → 서비스설정 → 소셜 로그인에서 Google 키를 등록하세요.')} />
                      )}
                      <SocialBtn icon="💛" label="카카오로 간편가입" color="#3C1E1E" bg="#FEE500" border="#FEE500"
                        onClick={kakaoConfigured ? loginWithKakao : () => authToast('관리자 → 서비스설정 → 소셜 로그인에서 카카오 키를 등록하세요.')} />
                      <SocialBtn icon="🟢" label="네이버로 간편가입" color="#fff" bg="#03C75A" border="#03C75A"
                        onClick={naverConfigured ? loginWithNaver : () => authToast('관리자 → 서비스설정 → 소셜 로그인에서 네이버 키를 등록하세요.')} />
                    </div>
