...
function DayDetail({ date, user, classes, onNav }) {
  ...
  const [supplyCheckModal, setSupplyCheckModal] = useState(null)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                            ...
                            return (
                              <tr key={stu.id} style={{ borderBottom: '1px solid #f3f4f6', background: idx%2===0 ? '#fff' : '#fafafa' }}>
                                ...
                                <td style={{ padding: '8px 12px', fontSize: '13px', fontWeight: 700, color: '#111827' }}>
                                  {showDAlert && (
                                    <div
                                      onClick={() => setSupplyCheckModal({ studentId: stu.id, classId: cls.id, productId: si.productId, alertLabel: dAlertLbl, studentName: stu.name })}
                                      style={{ marginBottom: '3px', fontSize: '10px', fontWeight: 700, color: '#ef4444', background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: '4px', padding: '2px 5px', whiteSpace: 'nowrap', display: 'inline-block', cursor: 'pointer' }}>
                                      ⚠️ {dAlertLbl}
                                    </div>
                                  )}
                                  <div>{stu.name}</div>
      ...
      {supplyCheckModal && (
        <SupplyCheckModal
          studentName={supplyCheckModal.studentName} alertLabel={supplyCheckModal.alertLabel}
...
export function Dashboard({ user, onNav }) {
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1400px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {/* 교구 준비/지급 체크 모달 (대시보드 상단 카드용) */}
      {dashScModal?.productId && (
        <SupplyCheckModal
          studentName={dashScModal.studentName} alertLabel={dashScModal.alertLabel}
