...
function DayDetail({ date, user, classes, onNav }) {
  ...
  useEffect(() => {
    ...
    try { setSpItems(SupplyItems.byTeacher(user.id)) } catch(e) {}
    try { setSpProds(SupplyProducts.byTeacher(user.id)) } catch(e) {}
    try { setSpProg(SupplyStudentProgress.byTeacher(user.id)) } catch(e) {}
    try { setSpChecks(SupplySessionChecks.byTeacher(user.id)) } catch(e) {}
  }, [date, user.id])
                        ...
                        return (
                          <tr key={stu.id} style={{ borderBottom:'1px solid #f3f4f6', background:idx%2===0?'#fff':'#fafafa' }}>
                            ...
                            <td style={{ padding:'8px 12px', fontSize:'13px', fontWeight:700, color:'#111827' }}>
                              ...
                              {hb&&(
                                <div style={{ display:'flex',gap:'3px',flexWrap:'wrap',marginTop:'3px' }}>
                                  ...
                                  {(stu.status==='cancel_before'||stu.status==='cancel_after')&&<span style={{ fontSize:'10px',fontWeight:700,padding:'1px 5px',borderRadius:'4px',background:'#fef2f2',border:'1px solid #fca5a5',color:'#dc2626' }}>{stu.status==='cancel_after'?'개강후 취소':'개강전 취소'}</span>}
                            ...
                            <td style={{ padding:'8px 12px', maxWidth:'150px' }}>
                              {ar?.note?<span style={{ fontSize:'11px',color:'#374151',background:'#fffbeb',padding:'2px 6px',borderRadius:'5px',border:'1px solid #fde68a',display:'block',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{ar.note}</span>
                                :<span style={{ fontSize:'11px',color:'#d1d5db' }}>-</span>}
                            </td>
                          </tr>
                        )
