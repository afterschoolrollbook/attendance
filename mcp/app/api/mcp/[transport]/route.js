...
//
// 방과후 출석부 블로그 자동화용 MCP 서버
// 카드뉴스 MCP와 동일한 구조 (Next.js App Router + mcp-handler)
//
// 툴 14개:
//   - get_publish_log      : 발행 기록 조회 (중복 방지, STEP 1에서 가장 먼저 호출)
//   - get_keyword_data     : 과목/역량 그룹별 찜 키워드 + TOP 키워드 조회
//   - search_keyword_data  : keyword_stats 전체 검색 (황금키워드 탐색)
//   - naver_keyword_volume : 네이버 실시간 검색량 조회
//   - save_keyword_data    : 조회 결과 캐시 저장
//   - pick_keyword         : 키워드 찜하기
//   - search_keyword_picks : 찜 키워드 전체 검색
//   - mark_keyword_used    : 찜 키워드 사용 처리
//   - suggest_feature      : 새 각도/주제 제안 기록
...
//   SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY   - 출석부 Supabase
//   NAVER_AD_API_KEY / NAVER_AD_SECRET_KEY / NAVER_AD_CUSTOMER_ID
//   NAVER_CLIENT_ID / NAVER_CLIENT_SECRET      - 블로그 문서수 조회 (선택)
//   MCP_SHARED_SECRET                          - 커넥터 보호용 비밀키
//
// claude.ai 커넥터 등록:
//   https://attendance-blog-mcp.vercel.app/api/mcp?key=MCP_SHARED_SECRET값
...
const baseHandler = createMcpHandler(
  (server) => {
    ...
    server.registerTool('get_keyword_data', {
      ...
      description: '특정 과목/역량 카테고리(hint)의 찜한 키워드와 TOP 검색량 키워드를 가져온다. 키워드 각도 확정에 사용한다.',
      inputSchema: {
    ...
    server.registerTool('naver_keyword_volume', {
      ...
      description: '네이버 검색광고 키워드도구로 키워드별 월간 검색량(PC/모바일)과 경쟁도를 실시간 조회한다. 조회 후 save_keyword_data로 캐시에 저장해둔다.',
      inputSchema: {
    ...
    server.registerTool('save_keyword_data', {
      ...
      description: 'naver_keyword_volume 조회 결과를 blog_keyword_stats에 저장한다. hint는 카테고리명으로 지정한다. 같은 hint+keyword는 최신 값으로 덮어쓴다.',
      inputSchema: {
    ...
    server.registerTool('pick_keyword', {
      ...
      description: '나중에 글로 쓰고 싶은 키워드를 찜해둔다. memo에 어떤 각도의 글을 쓸지 계획을 적어둔다.',
      inputSchema: {
    ...
    server.registerTool('search_keyword_picks', {
      ...
      description: '찜해둔 키워드를 카테고리 구분 없이 전체 열람한다. 기본은 미사용만 보여준다. 글감 정하기 전 한 번 호출해서 오늘 쓸 만한 게 있는지 확인한다.',
      inputSchema: {
      ...
      rows.forEach(p => {
        const usedNote = p.used_at ? ` · ✅ 사용됨(${p.used_at.slice(0, 10)}, ${p.used_in_title || ''})` : ''
    ...
    server.registerTool('mark_keyword_used', {
      title: '찜 키워드 사용 처리',
      description: '찜해둔 키워드를 실제 글에 썼을 때 호출해서 사용됨으로 표시한다. create_blog_post와 add_publish_log 직후 호출한다.',
      inputSchema: {
        group: z.string().describe('카테고리명'),
        keyword: z.string(),
    ...
    server.registerTool('suggest_feature', {
      title: '새 각도/주제 제안 기록',
      description: '특정 시리즈+카테고리에서 새로운 글 각도나 주제를 제안하고 싶을 때 기록한다. notes에 왜 이 각도가 좋은지 적는다.',
      inputSchema: {
    ...
    server.registerTool('get_feature_ideas', {
      ...
      inputSchema: {
        series: z.enum(SERIES_CODES).optional(),
    ...
    server.registerTool('add_publish_log', {
      title: '블로그 발행 기록 추가',
      description: '새로 작성한 글을 발행 기록에 남긴다. create_blog_post 직후 호출한다. 찜 키워드를 썼다면 memo에 남긴다.',
      inputSchema: {
        series: z.enum(SERIES_CODES).describe('시리즈 코드'),
        ...
        target_keyword: z.string().optional(), search_pc: z.number().optional(), search_mobile: z.number().optional(), search_total: z.number().optional(), competition: z.string().optional(),
      },
      annotations: { destructiveHint: false, idempotentHint: false },
    ...
    server.registerTool('create_blog_post', {
      title: '블로그 글 실제 발행',
      description: '작성한 글 본문을 blog_posts 테이블에 발행한다. 기본 status는 published(즉시 공개). 발행 후 반드시 add_publish_log와 mark_keyword_used도 호출한다.',
      inputSchema: {
      ...
      if (error) return { content: [{ type: 'text', text: `오류: ${error.message}` }], isError: true }
      const note = finalStatus === 'published' ? `✅ 발행 완료 — https://afterschoolrollbook.kr/blog/${slug}` : `✅ ${finalStatus === 'draft' ? '임시저장' : '예약'} 완료`
    ...
    server.registerTool('get_series_info', {
      ...
      inputSchema: { series_id: z.string().optional().describe('특정 시리즈/카테고리 ID. 비우면 전체') },
    }, async ({ series_id }) => {
  ...
  {},
  { basePath: '/api/mcp', maxDuration: 30, verboseLogs: true }
)
